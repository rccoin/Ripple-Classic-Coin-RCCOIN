...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Rccoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
   {"139.162.239.226","139.162.202.5"},{NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Rccoin is probably already running."), addrBind.ToString().c_str());
        else
